...
export default function DocumentTable({ documents, isLoading, onDelete, isAdmin }) {
  if (isLoading) {
    return (
  ...
  if (documents.length === 0) {
    return (
      <div className="text-center py-16 rounded-2xl border border-dashed border-ink/10 bg-white">
        ...
        <p className="text-sm text-ink-soft font-medium">No documents uploaded yet</p>
        <p className="text-xs text-ink-faint mt-1">Upload a PDF above to make it searchable.</p>
      </div>
    )
  }
              ...
              >
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-medium text-ink truncate max-w-[200px]">{doc.filename}</span>
                  </div>
                </td>
                ...
                <td className="px-5 py-3.5 text-ink-soft hidden sm:table-cell">{formatDate(doc.uploaded_at)}</td>
                <td className="px-5 py-3.5">
                  <span
                    className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${STATUS_STYLES[doc.status] || 'bg-ink/5 text-ink-soft'}`}
                  >
                    {doc.status}
                  </span>
                </td>
